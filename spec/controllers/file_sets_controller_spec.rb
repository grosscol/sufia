  ...
  describe "update" do
    ...
    context "when updating metadata" do
      ...
      it "spawns a content new version event job" do
        expect(ContentUpdateEventJob).to receive(:perform_later).with(file_set.id, 'jilluser@example.com').once
        allow(ContentNewVersionEventJob).to receive(:perform_later).with(file_set.id, 'jilluser@example.com')

        allow(CharacterizeJob).to receive(:perform_later).with(file_set.id)
        expect(CharacterizeJob).to receive(:perform_later).with(file_set.id)
        file = fixture_file_upload('/world.png', 'image/png')
    ...
    context "when updating the attached file" do
      it "spawns a content new version event job" do
        ...
        allow(CharacterizeJob).to receive(:perform_later).with(file_set.id)
        expect(CharacterizeJob).to receive(:perform_later).with(file_set.id).once
    ...
    it "spawns a virus check" do
      ...
      allow(CharacterizeJob).to receive(:perform_later).with(file_set.id)
      allow(CreateDerivativesJob).to receive(:perform_later).with(file_set.id)
      file = fixture_file_upload('/world.png', 'image/png')
      expect(ClamAV.instance).to receive(:scanfile).and_return(0)
      expect(CreateDerivativesJob).to receive(:perform_later).with(file_set.id).once
      expect(CharacterizeJob).to receive(:perform_later).with(file_set.id).once
